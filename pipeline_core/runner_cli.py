...
def _build(args: argparse.Namespace) -> tuple[str, int]:
    ...
    if not route_reason_by_id and compiled_profile is not None:
        try:
            compiled_plan = compile_run_plan(
                ...
                overrides=ControlOverrides(max_repair_attempts=args.max_repair_attempts),
                task=args.task,
                through=args.through,
            )
    ...
    if args.task is not None:
        declared = (
            compiled_plan.task(args.task).depends_on
            if compiled_plan is not None
            else next(t["depends_on"] for t in plan_tasks if t["id"] == args.task)
        )
    ...
    if args.dry_run:
        ...
        if post_task:
            post_task_lines = _post_task_gate_lines(
                release_policy=release_policy, wrapper_dir=wrapper_dir,
                output_count=output_count, approve_plan=args.approve_plan,
                approve_final_diff=args.approve_final_diff,
                commit_requested=args.commit or args.commit_approved_manifest,
            )
